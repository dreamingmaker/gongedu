...
완료 후 브라우저에서 `http://서버IP:3396` 로 접속합니다. (기본 포트: 3396)

> **포트를 변경하려면** `docker-compose.yml`의 `"3396:80"` 중 앞쪽 숫자를 원하는 포트로 수정하세요.
...
**서버에 직접 접근 가능한 경우** — 서버에서 브라우저로 `http://localhost:3396` 접속 후 로그인합니다.
...
이후 일상적인 관리는 해당 계정으로 하며, 접속은 실제 서버 IP(`http://서버IP:3396`)로 합니다.
